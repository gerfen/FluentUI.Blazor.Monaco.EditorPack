...
window.monacoMarkdownEditor = {
    ...
    // NEW: Optional pre-initialization hook
    beforeCreate: null,
    ...
    init: async function(containerId, initialContent, dotNetRef) {
        try {
            ...
            // NEW: Run pre-initialization hook if provided
            if (typeof window.monacoMarkdownEditor.beforeCreate === "function") {
                try {
                    await window.monacoMarkdownEditor.beforeCreate(monaco);
                    console.log("[MonacoMarkdown] beforeCreate hook executed");
                } catch (err) {
                    console.error("[MonacoMarkdown] Error in beforeCreate hook:", err);
                }
            }
            ...
            // NEW: Run post-initialization hook if provided
            if (typeof window.monacoMarkdownEditor.onInitialized === "function") {
                try {
                    await window.monacoMarkdownEditor.onInitialized(editor, monaco);
                    console.log("[MonacoMarkdown] onInitialized hook executed");
                } catch (err) {
                    console.error("[MonacoMarkdown] Error in onInitialized hook:", err);
                }
            }
    ...
    async initialize(editorId, initialContent, dotNetRef) {
        ...
        try {
            ...
            // Optional external initialization hook
            if (window.monacoMarkdownEditor.onInitialized) {
                try {
                    await window.monacoMarkdownEditor.onInitialized(editor, monaco);
                    console.log('[MonacoMarkdown] External initialization hook executed');
                } catch (err) {
                    console.error('[MonacoMarkdown] Error in external initialization hook:', err);
                }
            }
    ...
    /**
   * Get the initialized instance of the Monaco editor
   */
